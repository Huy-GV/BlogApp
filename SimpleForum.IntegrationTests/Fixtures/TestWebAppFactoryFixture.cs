...
namespace SimpleForum.IntegrationTests.Fixtures;

public class TestWebAppFactoryFixture : WebApplicationFactory<Program>, IAsyncLifetime
{
    private const string DatabaseName = "SimpleForumIntegrationTestDatabase";
    ...
    public TestWebAppFactoryFixture()
    {
        ...
        _mssqlContainer = new ContainerBuilder()
            .WithName($"SimpleForumTest-{Guid.NewGuid()}")
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        {
            x.RemoveAll<DbContextOptions<SimpleForumDbContext>>();

            var host = _mssqlContainer.Hostname;
            var port = _mssqlContainer.GetMappedPublicPort(MsSqlPort);
            x.AddDbContext<SimpleForumDbContext>(options =>
